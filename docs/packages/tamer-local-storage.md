...
```bash
npm i @tamer4lynx/tamer-local-storage@prerelease
pnpm add @tamer4lynx/tamer-local-storage@prerelease
bun add @tamer4lynx/tamer-local-storage@prerelease
```

Or: `t4l add tamer-local-storage`. Run **`t4l link`** after installing.

This package is **not** part of `t4l add-core`; add it explicitly if you need it.
